...
export default async function DocsPage({ searchParams }: { searchParams: Promise<{ project?: string, version?: string }> }) {
  ...
  if (projectId) {
    try {
      const resProj = await fetch(`http://localhost:4010/api/projects/${projectId}`, { 
        cache: 'no-store',
        headers 
      });
      ...
      const url = version 
        ? `http://localhost:4010/api/projects/${projectId}/endpoints?version=${version}`
        : `http://localhost:4010/api/projects/${projectId}/endpoints`;
