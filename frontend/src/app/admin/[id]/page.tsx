...
function UserDetailContent() {
  ...
  const fetchData = async (authToken: string) => {
    try {
      ...
      const usersRes = await fetch("http://localhost:4010/api/users", {
      ...
      // Fetch all projects
      const projectsRes = await fetch("http://localhost:4010/api/projects", {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      ...
      const accessRes = await fetch(`http://localhost:4010/api/users/${userId}/projects`, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
  ...
  const handleSaveProjects = async () => {
    try {
      ...
      for (const projectId of toRemove) {
        await fetch(`http://localhost:4010/api/projects/${projectId}/access/${userId}`, {
          method: "DELETE",
      ...
      for (const projectId of toAdd) {
        await fetch(`http://localhost:4010/api/projects/${projectId}/access`, {
          method: "POST",
          headers: {
