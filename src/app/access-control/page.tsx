...
export default function ProjectAccessManager() {
  ...
  const fetchProjectAccess = async (projectId: string, authToken: string) => {
    setError('');
    try {
      const res = await fetch(getApiUrl(`/api/projects/${projectId}/access`), {
        headers: { Authorization: `Bearer ${authToken}` }
      });
      if (!res.ok) {
        const errorText = await res.text();
    ...
    try {
      if (selectedProject === 'all') {
        const grantPromises = projects.map((project) =>
          fetch(getApiUrl(`/api/projects/${project.id}/access`), {
            method: 'POST',
            headers: {
        ...
        const res = await fetch(getApiUrl(`/api/projects/${selectedProject}/access`), {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
  ...
  const handleRevokeAccess = async (userId: string) => {
    ...
    try {
      const res = await fetch(
        getApiUrl(`/api/projects/${selectedProject}/access/${userId}`),
        {
          method: 'DELETE',
